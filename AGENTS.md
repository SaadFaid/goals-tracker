...
Two independent npm packages, **not** a workspace. Run commands in the correct directory:

- **Frontend** (repo root): `npm run dev` (Vite :5173), `npm run lint` (oxlint), `npm run build`
- **Server** (`server/`): `npm run dev` (Express :4000), `npm test` (node:test)

## Key gotchas

- **No ESLint** — linter is `oxlint` with React plugin rules. Run `npm run lint` from root.
- **Tests are pure** — `server/test/` uses `node:test` + `node:assert`. No Jest, no vitest, no DB needed. Run with `npm test` from `server/`.
- **Prisma is Postgres-only** — schema at `server/prisma/schema.prisma`. Run `npm run migrate` after schema changes. Run `npm run generate` to rebuild client.
- **Calc is duplicated on both sides, not shared** — `server/src/lib/calc.js` (server) and `src/lib/score.js` (client) are separate implementations kept in sync. Update both when changing calculation logic (server is the tested source of truth). Similarly `src/data/goals.js` mirrors `server/src/seed/defaultCategories.js` reward tier generation for guest mode.
